...
function PostStats({ post, userId }: PostStatsProps) {
  ...
  return (
    <div className="flex justify-between items-center z-20">
      ...
      <div className="flex gap-2">{isSavingPost || isDeletingSaved ? <Loader /> : <img src={`${isSaved ? "/assets/icons/saved.svg" : "/assets/icons/save.svg"}`} alt="Like" width={20} height={20} onClick={handleSavePost} className="cursor-pointer" />}</div>
    </div>
  );
}
